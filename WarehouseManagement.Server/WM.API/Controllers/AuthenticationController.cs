...
using WM.Entity.DTOs.AuthenticationDTO;
using Microsoft.EntityFrameworkCore;
using WM.Shared.Constants;
...
namespace WM.API.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public AuthenticationController(WarehouseManagementContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        public async Task<IActionResult> ResetPassword(string token, string newpwd)
        {
            try
            {
                ...
                if (emailtoken != null && !emailtoken.IsUsed && !emailtoken.IsRevoked && RegexConstant.validateGuidRegex.IsMatch(newpwd))
                {
        ...
        public async Task<IActionResult> ResetPasswordByEmail(string email)
        {
            try
            {
                ...
                if (user != null)
                {
                    ...
                    string url = UrlConstant.ApiBaseUrl + "/set-password?token=" + token;
                    string urlNotMe = UrlConstant.ApiBaseUrl + "/not-me?token=" + token;
        ...
        public async Task<IActionResult> ResendEmail(string email)
        {
            try
            {
                ...
                {
                    var token = TokenHelper.GenerateRandomToken(64);
                    string url = UrlConstant.ApiBaseUrl + "/verification-success?token=" + token;
                    MailMessage mm = new MailMessage("WarehouseManagementystem@gmail.com", email);
        ...
        public async Task<IActionResult> SignUp(SignUpDTO model)
        {
            try
            {
                ...
                if ((user == null || (user != null && !(user.StatusId ==1) )) && RegexConstant.validateGuidRegex.IsMatch(model.Password))
                {
                    var token = TokenHelper.GenerateRandomToken(64);
